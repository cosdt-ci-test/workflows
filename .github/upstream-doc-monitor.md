...
- `owner`：该条目产生的工单会 **@owner**（对任何 GitHub 用户生效）并尽力设为 assignee（仅仓库协作者可设，非协作者自动跳过）。**缺省则工单不 @人**——强烈建议每条目填写
...
改条目的 `owner` 字段即可。已 open 的工单不受影响（处理人看后续评论的 @ 提及）；新事件的新工单/新评论 @ 新 owner。
