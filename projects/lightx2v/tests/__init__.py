...
Mirror of ``projects/xtuner/tests/__init__.py``: the layout depth
(``tests/ -> <project>/ -> projects/ -> workflows/``) is identical, so
``parents[3]`` resolves to the repo root in both projects and the same
bootstrap block is reused verbatim.
"""
...
_REPO_ROOT = Path(__file__).resolve().parents[3]  # tests/ -> lightx2v/ -> projects/ -> workflows/
_SRC = _REPO_ROOT / 'src'
