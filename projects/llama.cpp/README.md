...
- 四个 `profile`，由 `setup_example.sh` 的 `setup_<profile>` 函数解释。未知 profile 在 cmake / 下载之前非 0 退出。
  ...
  - `cann-diffusion`：同样开 CANN，编 `llama-diffusion-cli`。Dream 架构 GGUF 不塞进 `cann`，避免每条 simple/batched 都拉 8GB。ModelScope 没有这份 GGUF；`hf-mirror` 会 302 到美国 CDN，国内 runner 下不下来。所以单独有一个前置 job `prepare-dream-gguf`：从 ModelScope `Dream-org/Dream-v0-Instruct-7B` 把官方 safetensors 先落到 `/root/.cache/cosdt-ci-test/llama.cpp/Dream-v0-Instruct-7B-hf/`（按文件大小跳过已下完的分片），再用这次 checkout 的 `convert_hf_to_gguf.py` 转成 `Dream-v0-Instruct-7B.Q8_0.gguf`。HF 目录转换失败也留着，下次不用重下。GitHub Actions 的 `needs` 只能挂整 job，不能挂 matrix 的某一行，所以 `examples/diffusion` 从主 matrix 拆到 `run-diffusion`，只有它等准备 job；其余 example 仍并行。容器必须 `--volume=/root/.cache/cosdt-ci-test:/root/.cache/cosdt-ci-test`：SFS Turbo 挂在宿主机 `/root/.cache`，不绑进去容器里看不见。不要用 `actions/cache` 或 artifact 传这 8GB。`setup_cann-diffusion` 只消费缓存里的 GGUF，缺了或文件头不是 `GGUF` 是 setup 失败（看护噪音），不会改用 Qwen。
