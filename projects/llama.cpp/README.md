...
- 四个 `profile`，由 `setup_example.sh` 的 `setup_<profile>` 函数解释。未知 profile 在 cmake / 下载之前非 0 退出。
  ...
  - `cann-diffusion`：同样开 CANN，编 `llama-diffusion-cli`。Dream 架构 GGUF 另下，不塞进 `cann`，避免每条 simple/batched 都拉 8GB。现用钉死 URL：`https://hf-mirror.com/mradermacher/Dream-v0-Instruct-7B-GGUF/resolve/main/Dream-v0-Instruct-7B.Q8_0.gguf`（约 8.1GB）。ModelScope 上没有这份副本；该仓库也没有 Q4_0。下载失败或文件头不是 `GGUF` 是 setup 失败（看护噪音），不会改用 Qwen。
