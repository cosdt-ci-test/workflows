...
# Stop only process groups named in these files. Never pkill.
# The runner may host other inference jobs.
_OWNED_PID_FILES = (
    Path('/root/llm-d/envoy.pid'),
    Path('/root/llm-d/epp.pid'),
    Path('/root/llm-d/vllm.pid'),
)


def _stop_pid_file(pid_file: Path, wait_s: float = 15.0) -> None:
...
class TestQuickStartAscend(MarkdownDocTestBase, unittest.TestCase):
    ...
    def post_process(self) -> None:
        for pid_file in _OWNED_PID_FILES:
