...
> 编译时间受 CPU 核数与是否启用 world 影响：单核 `make` 大约 1.5 小时；8 核并行约 20 分钟。CI runner 32 核、`-j$(nproc)`/`nproc/2` 都会让 dnn 模板大 TU（matmul / dft / reshape2 / slice2 / pad2 等 layer）`cc1plus` 被 OOM kill，实测只有 `nproc/4`（~8 并行）能跑通。
