...
- 镜像预装 torch 2.10.0 + torch_npu 2.10.0 + sglang 0.5.18 + CANN 9.1.0 + Python 3.11.15（`check-torch` 步校验；specforge 源码 `pip install --no-deps .` 跳过上游 `pyproject.toml` 的 sglang==0.5.14 pin）；再装 modelscope 1.37.0 + mooncake-transfer-engine 0.3.13；
