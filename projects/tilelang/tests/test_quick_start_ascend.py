...
Document under test: ``projects/tilelang/docs/Quick-start-Ascend.md``
...
class TestQuickStartAscend(MarkdownDocTestBase, unittest.TestCase):
    ...
    Scope: env check + source build of the tilelang-ascend wheel
    (TVM submodule + C++ compile) + a GEMM operator run on a single
    Ascend NPU card. Pure operator workload — no model weights, hence
    no ModelScope cache handling in this class.
    """

    # 60 min per command: long enough for the --recursive clone (TVM
    # submodule) plus the C++ wheel build on the first run, which needs
    ...
    # the doc lives at projects/tilelang/docs/Quick-start-Ascend.md and
    ...
    # (subprocess.run inherits parent env by default) see it. tilelang's
    # build requirements shouldn't pull CUDA, but the torch stack the
    # build resolves against must stay NPU-only.
    _CUDA_CONSTRAINTS = (
        'cuda-toolkit<0',
        'cuda-python<0',
        'cuda-bindings<0',
    ...
    _CONSTRAINTS_FILE = '/tmp/tilelang_npu_constraints.txt'
    ...
    # Default path, tied to the container image pinned by the ``image:``
    # input of ``tilelang-quick-start.yml``. The build script additionally
    # needs ``ASCEND_HOME_PATH``; when the toolkit is not at the default
    # location (observed on some runner images), ``_resolve_ascend_home``
    # probes alternates and exports it for the doc's shell blocks.
    _CANN_SET_ENV = '/usr/local/Ascend/ascend-toolkit/set_env.sh'
    _ASCEND_TOOLKIT_ROOT = '/usr/local/Ascend/ascend-toolkit'
    ...
    def pre_process(self) -> str:
        ...
        the doc at ``projects/tilelang/docs/Quick-start-Ascend.md``
    ...
    # ----------------------------------------------------------
    # CANN toolkit discovery for ASCEND_HOME_PATH
    # ----------------------------------------------------------

    @classmethod
    def _resolve_ascend_home(cls) -> str | None:
        """Return an existing CANN toolkit dir (contains ``include/``).

        Probe order: current ``ASCEND_HOME_PATH`` (if already valid),
        the ``latest`` symlink, then the highest versioned directory
        under the toolkit root. Returns ``None`` when nothing matches —
        the doc then falls back to its documented default via
        ``${ASCEND_HOME_PATH:-...}``.
        """
        current = os.environ.get('ASCEND_HOME_PATH')
        root = Path(cls._ASCEND_TOOLKIT_ROOT)
        candidates: list[Path] = []
        if current:
            candidates.append(Path(current))
        candidates.append(root / 'latest')
        if root.is_dir():
            candidates.extend(
                p for p in sorted(root.iterdir(), reverse=True) if p.is_dir()
            )
        for cand in candidates:
            if cand.is_dir() and (cand / 'include').is_dir():
                return str(cand)
        return None

    # ----------------------------------------------------------
    # prepare_environment: CANN env + ASCEND_HOME_PATH + CUDA
    # constraints + uv + torch stack probe
    # ----------------------------------------------------------

    @classmethod
    def prepare_environment(cls) -> None:
        """Source CANN env + resolve ASCEND_HOME_PATH + write CUDA
        exclusion list + install uv + torch stack probe.

        The doc's build block is the single source of truth for how
        tilelang-ascend gets built and installed; this class only
        handles ``torch`` / ``torch_npu`` here (via the cluster cache +
        Huawei ascend dual-source) and the environment the build needs.
        ...
        # 0b) ASCEND_HOME_PATH for the tilelang build script: probe the
        # toolkit location and export it so the doc's shell blocks (and
        # build_wheel_ascend.sh) inherit it via subprocess env.
        ascend_home = cls._resolve_ascend_home()
        if ascend_home:
            os.environ['ASCEND_HOME_PATH'] = ascend_home
            print(f'setup: ASCEND_HOME_PATH={ascend_home}')
        else:
            print(
                'setup: ASCEND_HOME_PATH not resolved by probe; '
                'doc falls back to its documented default'
            )
        ...
        # 2) uv: kept for parity with the other projects' setup (the
        # doc flow itself uses ``python -m pip``). Inherit
    ...
    def setUpClass(cls) -> None:
        """Run env setup once per test class: CANN env + ASCEND_HOME_PATH +
        CUDA constraints + uv + torch stack.
