...
class TestQuickStartAscend(MarkdownDocTestBase, unittest.TestCase):
    ...
    # num_workers=0 (main-process data loading, see the doc's note on
    # the fork-safety workaround), plus stage 5 decodes 7176 test utts
    # in 4 modes. 1h was enough for the mock-data era only.
    ...
    def prepare_environment(cls) -> None:
        ...
        _PROBE_SCRIPT = (
            ...
            "and torch_npu.__version__.startswith('2.2') "
            "else 1)"
        )
        ...
        if probe.returncode == 0:
            ...
            print('setup: installing torch==2.2.0 torch-npu==2.2.0.post2')
            subprocess.run(
                [
                    'python', '-m', 'pip', 'install',
                    'torch==2.2.0', 'torch-npu==2.2.0.post2',
                ],
                check=True,
            )
